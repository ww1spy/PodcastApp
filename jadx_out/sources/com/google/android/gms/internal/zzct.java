package com.google.android.gms.internal;

import android.app.Activity;
import android.content.Context;
import android.support.annotation.VisibleForTesting;
import android.support.v4.media.session.PlaybackStateCompat;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

/* loaded from: classes.dex */
public class zzct extends zzcs {
    private static final String TAG = "zzct";

    @VisibleForTesting
    private static boolean initialized = false;
    private static long startTime;
    private static volatile zzdm zzagq;
    private static Object zzahp = new Object();
    protected boolean zzahq;
    private String zzahr;
    private boolean zzahs;
    private boolean zzaht;

    @VisibleForTesting
    private zzdu zzahu;

    /* JADX INFO: Access modifiers changed from: protected */
    public zzct(Context context, String str, boolean z) {
        super(context);
        this.zzahq = false;
        this.zzahs = false;
        this.zzaht = false;
        this.zzahr = str;
        this.zzahq = z;
    }

    private static zzdr zza(zzdm zzdmVar, MotionEvent motionEvent, DisplayMetrics displayMetrics) throws zzdj {
        Method zza = zzdmVar.zza("LLWEGIcMoDZoYXR7U5oWQiG0eqgHAi5y99iNxWUoUCrqQe4SBuROX6hcHHgPH+SM", "O3KEK4C8mMJ55X7+rKhyFI2gabDgFgd1qlmX9Na34/M=");
        if (zza == null || motionEvent == null) {
            throw new zzdj();
        }
        try {
            return new zzdr((String) zza.invoke(null, motionEvent, displayMetrics));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzdj(e);
        }
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public static synchronized void zza(Context context, boolean z) {
        synchronized (zzct.class) {
            if (!initialized) {
                startTime = System.currentTimeMillis() / 1000;
                zzagq = zzb(context, z);
                initialized = true;
            }
        }
    }

    private static zzdm zzb(Context context, boolean z) {
        if (zzagq == null) {
            synchronized (zzahp) {
                if (zzagq == null) {
                    zzdm zza = zzdm.zza(context, "fmyYtvp6GdfV8aXECqySf5usPZLp4lFIXsdmCOa6f3I=", "", z);
                    if (zza.isInitialized()) {
                        zza.zza("b0nS9elqzY5/VJmQt0NlC62dB7yCaB6LeKUB+YXUuuDDycP369v3LvQySmqqDQ6y", "vfxPDbj+Gh4UJloJgP7FDEw0pUJBChFkLoiSG9W3S+I=", Context.class);
                        zza.zza("Nt7RB2lfn/Td3UbDXq8jAOuCI/Ll2Hp2FhUggwUDXFsb8F7bope53TUJgWthHbTZ", "PQ5SSaxTSuJzj647muMyUtZ+uYgTQgJ31c5fk186jfk=", Context.class);
                        zza.zza("xjAQSkwzRxajmgCbKHDMDKALp7GNWm8BXcaw0/hWyFOZu/eOjfjL582/zkzt4p+j", "YBerbJehoFKiWWatcGr+va4R/rOgEqbq5xzJrvD/KqU=", Context.class);
                        zza.zza("Yt5XpSAtwgzkLrmXQYrSRr3fMYrlcB9+S6GNMxeM/YPVx9v5CKvi8nPmgPLSujhj", "dR638FFygwsrTt/QflyOMGvXjf1GSNZ7znIlrDruhqQ=", Context.class);
                        zza.zza("dn8+8pHjJ2wj5Gp1r/T7fEkOwzyz7KKq5AugUUrNOwEHnTcvhTP4W3g7fxLqTpFB", "c6GziBvAtJVQl8145kYNIiXglYZwc9tWD7nqr7hLy/A=", Context.class);
                        zza.zza("aPsscg+Qxy0iqv59xAu19ukICD9tJAt4EQOJODpj5Ak49GynbXKvHyrrGvZ8bZTH", "gvEkKyx3YjkW3LmeffZWM+wAodTj/mcVSvq1iQ03Xm0=", Context.class);
                        zza.zza("VN0mjb52HdSTqOivTj5aML5bEfMpu2JdQlvUopb/cybtBLaro7TgCOYdeQg7NyAj", "gp0cIgUUdAIZbRXtUI5gQGevY7Ql5nC5BzKn6LnObLY=", Context.class);
                        zza.zza("cma5ZMQhKxelkLoCs6AZIf+jHy8LfWoLKTm3I25QQ7y8l7CIShnEfyEmDBtS6X1R", "8HMIy/Wfi2fsqmlkEx1MOaO07gLN9KPbPeJd9GTSqvM=", Context.class);
                        zza.zza("LLWEGIcMoDZoYXR7U5oWQiG0eqgHAi5y99iNxWUoUCrqQe4SBuROX6hcHHgPH+SM", "O3KEK4C8mMJ55X7+rKhyFI2gabDgFgd1qlmX9Na34/M=", MotionEvent.class, DisplayMetrics.class);
                        zza.zza("Ncqp5H+22w9W3L8txFxikPL++CvvT4UACmJT2y9HNWRLTaCz/GhnD086ih+pDclQ", "oClLCKzkLLWAqjtGJeQeLZr6xaB9RrnEuNDXspc7PYI=", MotionEvent.class, DisplayMetrics.class);
                        zza.zza("Be4XNeKe48otW84OnyuJ3t3qbzO8QDmRqoj9GLjPbqCKHI4EjzEK31ZhyeIA63g7", "rw94Vrx/byjpqg0QbJ3qk6x9nbH4tHcDnVkhwVzqmY0=", new Class[0]);
                        zza.zza("fuI1JBLz73BbSdCY+eHqDP1u6baqogxj7PqSFpzZgrAG3XsVxk7kH3pFFSmoNQQw", "o4gCRP8pd3YBLYpyJe+pyrNh2wHBW5ZmIwpxvbEaiEk=", new Class[0]);
                        zza.zza("AifE6OWHQ8zRmArdaoYy3/wDT3Tse3b0SsXzLMY2JhYTdkCeDJ1ty8MUlcSgr6P8", "cpk+vnXicdAmu6rqNpyRsH8Z9xFtWGhKh1PSt7R1wgo=", new Class[0]);
                        zza.zza("0MHmFOerm+OLO7DlwBWYWOfS31i3abNb7NII/p/cozDrwy0Qt4Hz6KjLmcNA2Uu1", "dd/st6q19rXcYPAfnaAl9NStsvK8rw8184B9GIFsJ/U=", new Class[0]);
                        zza.zza("Kl5uWPRlOPd31gHSoBTxQgFz93Wgsh3JQ5+syp0rqA1C1BUyIJ1EfI70uCpXJEXY", "MmO0rCvC3l83tbAoIwnFF0x6ASHVe9zZRiunMzgf61w=", new Class[0]);
                        zza.zza("Jr72ymQHv5OslUBKT3bZCPBk5tCtKWt432zERI2guaDk8CCuhfLPuPGiolFM9O2V", "zYGiKyXCQARtrMz4+R+cwjpghR9+uCtdZ+gYsXH0O9M=", new Class[0]);
                        zza.zza("fCahZmmokRlkTziAR2h88rL/W8OULXQVU2c0PgTQTnRvhkE51fPDmWahg/1J8rwq", "xZw7LXXqVYeXIr/mYvF7euX+S5ZW/jdzGMuluL/FTHw=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        zza.zza("wflZ5ZrFWxKN35Rs/obkm9xcWW4uwhgDI7R93vb2gpXPjsYJYEKVt9jrMQOUQeAA", "cXUR3QZnvsd8QFJrigAEcnhF5qRi6A1WjhzghuHtPIs=", StackTraceElement[].class);
                        zza.zza("nlNsfD0KfLAOzjZnsadcRaT7rHxwp4GC5EZVTVhnwe/pnADxonobSxOiLY3KGgZG", "yEGznoCvboNomg+D/tjSW+eddhf/oFTqSBHDCsg5uVM=", View.class, DisplayMetrics.class);
                        zza.zza("MT46CBRZPOofnotcmzgUNA75mBeAcqcqzo80kU1Wki/M3T0+Ilf2BPD/W1SRuWf8", "85aVi3SJCzURUruw80xwOCJlrH71CKBJVp0z1HeuVQI=", Context.class, Boolean.TYPE);
                        zza.zza("olucaRzp8OAM7j17mlu+MNg8QWI1/0HYxPp/zXgvUcGiOa2hMytoVkpTSVdUA6zj", "kX7NAbLZCz/dSKM5Fw5tfFEDlfsFCqwNHFXHrnBBf44=", View.class, Activity.class);
                        zza.zza("/SomwDyLd+iYT1TBJ8yczLZbKQsVJsVm19/eLPmi+tCXN71809kfhdY0YXdgoZoi", "xQNz5sgiknpAv6ackbFAd0cOAtMz1Xn/IL6OG4Bwh/4=", Long.TYPE);
                    }
                    zzagq = zza;
                }
            }
        }
        return zzagq;
    }

    private static void zzb(List<Callable<Void>> list) {
        ExecutorService executorService;
        if (zzagq == null || (executorService = zzagq.getExecutorService()) == null || list.isEmpty()) {
            return;
        }
        try {
            executorService.invokeAll(list, ((Long) zzlc.zzio().zzd(zzoi.zzbri)).longValue(), TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            Log.d(TAG, String.format("class methods got exception: %s", zzds.zza(e)));
        }
    }

    @Override // com.google.android.gms.internal.zzcs
    protected final long zza(StackTraceElement[] stackTraceElementArr) throws zzdj {
        Method zza = zzagq.zza("wflZ5ZrFWxKN35Rs/obkm9xcWW4uwhgDI7R93vb2gpXPjsYJYEKVt9jrMQOUQeAA", "cXUR3QZnvsd8QFJrigAEcnhF5qRi6A1WjhzghuHtPIs=");
        if (zza == null || stackTraceElementArr == null) {
            throw new zzdj();
        }
        try {
            return new zzdk((String) zza.invoke(null, stackTraceElementArr)).zzail.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzdj(e);
        }
    }

    /* JADX WARN: Multi-variable type inference failed */
    /* JADX WARN: Type inference failed for: r1v51, types: [java.util.List] */
    @Override // com.google.android.gms.internal.zzcs
    protected final zzba zza(Context context, View view, Activity activity) {
        ArrayList arrayList;
        zzba zzbaVar = new zzba();
        if (!TextUtils.isEmpty(this.zzahr)) {
            zzbaVar.zzcu = this.zzahr;
        }
        zzdm zzb = zzb(context, this.zzahq);
        if (zzb.isInitialized()) {
            try {
                zzdr zza = zza(zzb, this.zzagw, this.zzaho);
                zzbaVar.zzdm = zza.zzajl;
                zzbaVar.zzdn = zza.zzajm;
                zzbaVar.zzdo = zza.zzajn;
                if (this.zzahn) {
                    zzbaVar.zzea = zza.zzfl;
                    zzbaVar.zzeb = zza.zzfj;
                }
            } catch (zzdj unused) {
            }
            zzbb zzbbVar = new zzbb();
            if (this.zzagy > 0 && zzds.zza(this.zzaho)) {
                zzbbVar.zzft = Long.valueOf(zzds.zza(this.zzahf, this.zzaho));
                if (((Boolean) zzlc.zzio().zzd(zzoi.zzbro)).booleanValue()) {
                    zzbbVar.zzfu = Long.valueOf(zzds.zza(this.zzahk - this.zzahi, this.zzaho));
                    zzbbVar.zzfv = Long.valueOf(zzds.zza(this.zzahl - this.zzahj, this.zzaho));
                    zzbbVar.zzfy = Long.valueOf(zzds.zza(this.zzahi, this.zzaho));
                    zzbbVar.zzfz = Long.valueOf(zzds.zza(this.zzahj, this.zzaho));
                }
                if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrp)).booleanValue() && this.zzahn && this.zzagw != null) {
                    long zza2 = zzds.zza(((this.zzahi - this.zzahk) + this.zzagw.getRawX()) - this.zzagw.getX(), this.zzaho);
                    if (zza2 != 0) {
                        zzbbVar.zzfw = Long.valueOf(zza2);
                    }
                    long zza3 = zzds.zza(((this.zzahj - this.zzahl) + this.zzagw.getRawY()) - this.zzagw.getY(), this.zzaho);
                    if (zza3 != 0) {
                        zzbbVar.zzfx = Long.valueOf(zza3);
                    }
                }
            }
            try {
                zzdr zzb2 = zzb(this.zzagw);
                zzbbVar.zzdm = zzb2.zzajl;
                zzbbVar.zzdn = zzb2.zzajm;
                zzbbVar.zzfo = zzb2.zzajn;
                if (this.zzahn) {
                    zzbbVar.zzfj = zzb2.zzfj;
                    zzbbVar.zzfl = zzb2.zzfl;
                    zzbbVar.zzfn = Integer.valueOf(zzb2.zzajo.longValue() != 0 ? 1 : 0);
                    if (this.zzagz > 0) {
                        zzbbVar.zzfk = zzds.zza(this.zzaho) ? Long.valueOf(Math.round(this.zzahe / this.zzagz)) : null;
                        zzbbVar.zzfm = Long.valueOf(Math.round(this.zzahd / this.zzagz));
                    }
                    zzbbVar.zzfq = zzb2.zzfq;
                    zzbbVar.zzfp = zzb2.zzfp;
                    zzbbVar.zzfr = Integer.valueOf(zzb2.zzajr.longValue() != 0 ? 1 : 0);
                }
            } catch (zzdj unused2) {
            }
            if (this.zzahc > 0) {
                zzbbVar.zzfs = Long.valueOf(this.zzahc);
            }
            zzbaVar.zzer = zzbbVar;
            if (this.zzagy > 0) {
                zzbaVar.zzef = Long.valueOf(this.zzagy);
            }
            if (this.zzagz > 0) {
                zzbaVar.zzee = Long.valueOf(this.zzagz);
            }
            if (this.zzaha > 0) {
                zzbaVar.zzed = Long.valueOf(this.zzaha);
            }
            if (this.zzahb > 0) {
                zzbaVar.zzeg = Long.valueOf(this.zzahb);
            }
            try {
                int size = this.zzagx.size() - 1;
                if (size > 0) {
                    zzbaVar.zzes = new zzbb[size];
                    for (int i = 0; i < size; i++) {
                        zzdr zza4 = zza(zzagq, this.zzagx.get(i), this.zzaho);
                        zzbb zzbbVar2 = new zzbb();
                        zzbbVar2.zzdm = zza4.zzajl;
                        zzbbVar2.zzdn = zza4.zzajm;
                        zzbaVar.zzes[i] = zzbbVar2;
                    }
                }
            } catch (zzdj unused3) {
                zzbaVar.zzes = null;
            }
            arrayList = new ArrayList();
            if (zzb.getExecutorService() != null) {
                int zzab = zzb.zzab();
                arrayList.add(new zzef(zzb, zzbaVar));
                arrayList.add(new zzei(zzb, "AifE6OWHQ8zRmArdaoYy3/wDT3Tse3b0SsXzLMY2JhYTdkCeDJ1ty8MUlcSgr6P8", "cpk+vnXicdAmu6rqNpyRsH8Z9xFtWGhKh1PSt7R1wgo=", zzbaVar, zzab, 1));
                arrayList.add(new zzed(zzb, "Be4XNeKe48otW84OnyuJ3t3qbzO8QDmRqoj9GLjPbqCKHI4EjzEK31ZhyeIA63g7", "rw94Vrx/byjpqg0QbJ3qk6x9nbH4tHcDnVkhwVzqmY0=", zzbaVar, startTime, zzab, 25));
                arrayList.add(new zzec(zzb, "0MHmFOerm+OLO7DlwBWYWOfS31i3abNb7NII/p/cozDrwy0Qt4Hz6KjLmcNA2Uu1", "dd/st6q19rXcYPAfnaAl9NStsvK8rw8184B9GIFsJ/U=", zzbaVar, zzab, 44));
                arrayList.add(new zzeh(zzb, "dn8+8pHjJ2wj5Gp1r/T7fEkOwzyz7KKq5AugUUrNOwEHnTcvhTP4W3g7fxLqTpFB", "c6GziBvAtJVQl8145kYNIiXglYZwc9tWD7nqr7hLy/A=", zzbaVar, zzab, 12));
                arrayList.add(new zzej(zzb, "aPsscg+Qxy0iqv59xAu19ukICD9tJAt4EQOJODpj5Ak49GynbXKvHyrrGvZ8bZTH", "gvEkKyx3YjkW3LmeffZWM+wAodTj/mcVSvq1iQ03Xm0=", zzbaVar, zzab, 3));
                arrayList.add(new zzeg(zzb, "Kl5uWPRlOPd31gHSoBTxQgFz93Wgsh3JQ5+syp0rqA1C1BUyIJ1EfI70uCpXJEXY", "MmO0rCvC3l83tbAoIwnFF0x6ASHVe9zZRiunMzgf61w=", zzbaVar, zzab, 22));
                arrayList.add(new zzeb(zzb, "Yt5XpSAtwgzkLrmXQYrSRr3fMYrlcB9+S6GNMxeM/YPVx9v5CKvi8nPmgPLSujhj", "dR638FFygwsrTt/QflyOMGvXjf1GSNZ7znIlrDruhqQ=", zzbaVar, zzab, 5));
                arrayList.add(new zzeq(zzb, "VN0mjb52HdSTqOivTj5aML5bEfMpu2JdQlvUopb/cybtBLaro7TgCOYdeQg7NyAj", "gp0cIgUUdAIZbRXtUI5gQGevY7Ql5nC5BzKn6LnObLY=", zzbaVar, zzab, 48));
                arrayList.add(new zzdy(zzb, "cma5ZMQhKxelkLoCs6AZIf+jHy8LfWoLKTm3I25QQ7y8l7CIShnEfyEmDBtS6X1R", "8HMIy/Wfi2fsqmlkEx1MOaO07gLN9KPbPeJd9GTSqvM=", zzbaVar, zzab, 49));
                arrayList.add(new zzen(zzb, "Jr72ymQHv5OslUBKT3bZCPBk5tCtKWt432zERI2guaDk8CCuhfLPuPGiolFM9O2V", "zYGiKyXCQARtrMz4+R+cwjpghR9+uCtdZ+gYsXH0O9M=", zzbaVar, zzab, 51));
                arrayList.add(new zzem(zzb, "wflZ5ZrFWxKN35Rs/obkm9xcWW4uwhgDI7R93vb2gpXPjsYJYEKVt9jrMQOUQeAA", "cXUR3QZnvsd8QFJrigAEcnhF5qRi6A1WjhzghuHtPIs=", zzbaVar, zzab, 45, new Throwable().getStackTrace()));
                arrayList.add(new zzer(zzb, "nlNsfD0KfLAOzjZnsadcRaT7rHxwp4GC5EZVTVhnwe/pnADxonobSxOiLY3KGgZG", "yEGznoCvboNomg+D/tjSW+eddhf/oFTqSBHDCsg5uVM=", zzbaVar, zzab, 57, view));
                if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrk)).booleanValue()) {
                    arrayList.add(new zzel(zzb, "MT46CBRZPOofnotcmzgUNA75mBeAcqcqzo80kU1Wki/M3T0+Ilf2BPD/W1SRuWf8", "85aVi3SJCzURUruw80xwOCJlrH71CKBJVp0z1HeuVQI=", zzbaVar, zzab, 61));
                }
                if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrl)).booleanValue()) {
                    arrayList.add(new zzdx(zzb, "olucaRzp8OAM7j17mlu+MNg8QWI1/0HYxPp/zXgvUcGiOa2hMytoVkpTSVdUA6zj", "kX7NAbLZCz/dSKM5Fw5tfFEDlfsFCqwNHFXHrnBBf44=", zzbaVar, zzab, 62, view, activity));
                }
                if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrm)).booleanValue()) {
                    arrayList.add(new zzeo(zzb, "/SomwDyLd+iYT1TBJ8yczLZbKQsVJsVm19/eLPmi+tCXN71809kfhdY0YXdgoZoi", "xQNz5sgiknpAv6ackbFAd0cOAtMz1Xn/IL6OG4Bwh/4=", zzbaVar, zzab, 53, this.zzahu));
                }
            }
        } else {
            zzbaVar.zzds = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            arrayList = Arrays.asList(new zzef(zzb, zzbaVar));
        }
        zzb(arrayList);
        return zzbaVar;
    }

    @Override // com.google.android.gms.internal.zzcs
    protected final zzba zza(Context context, zzax zzaxVar) {
        zzba zzbaVar = new zzba();
        if (!TextUtils.isEmpty(this.zzahr)) {
            zzbaVar.zzcu = this.zzahr;
        }
        zzdm zzb = zzb(context, this.zzahq);
        if (zzb.getExecutorService() != null) {
            zzb(zza(zzb, zzbaVar, (zzax) null));
        }
        return zzbaVar;
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public List<Callable<Void>> zza(zzdm zzdmVar, zzba zzbaVar, zzax zzaxVar) {
        int zzab = zzdmVar.zzab();
        ArrayList arrayList = new ArrayList();
        if (!zzdmVar.isInitialized()) {
            zzbaVar.zzds = Long.valueOf(PlaybackStateCompat.ACTION_PREPARE);
            return arrayList;
        }
        arrayList.add(new zzea(zzdmVar, "fCahZmmokRlkTziAR2h88rL/W8OULXQVU2c0PgTQTnRvhkE51fPDmWahg/1J8rwq", "xZw7LXXqVYeXIr/mYvF7euX+S5ZW/jdzGMuluL/FTHw=", zzbaVar, zzab, 27, zzaxVar));
        arrayList.add(new zzed(zzdmVar, "Be4XNeKe48otW84OnyuJ3t3qbzO8QDmRqoj9GLjPbqCKHI4EjzEK31ZhyeIA63g7", "rw94Vrx/byjpqg0QbJ3qk6x9nbH4tHcDnVkhwVzqmY0=", zzbaVar, startTime, zzab, 25));
        arrayList.add(new zzei(zzdmVar, "AifE6OWHQ8zRmArdaoYy3/wDT3Tse3b0SsXzLMY2JhYTdkCeDJ1ty8MUlcSgr6P8", "cpk+vnXicdAmu6rqNpyRsH8Z9xFtWGhKh1PSt7R1wgo=", zzbaVar, zzab, 1));
        arrayList.add(new zzek(zzdmVar, "xjAQSkwzRxajmgCbKHDMDKALp7GNWm8BXcaw0/hWyFOZu/eOjfjL582/zkzt4p+j", "YBerbJehoFKiWWatcGr+va4R/rOgEqbq5xzJrvD/KqU=", zzbaVar, zzab, 31));
        arrayList.add(new zzep(zzdmVar, "fuI1JBLz73BbSdCY+eHqDP1u6baqogxj7PqSFpzZgrAG3XsVxk7kH3pFFSmoNQQw", "o4gCRP8pd3YBLYpyJe+pyrNh2wHBW5ZmIwpxvbEaiEk=", zzbaVar, zzab, 33));
        arrayList.add(new zzdz(zzdmVar, "Nt7RB2lfn/Td3UbDXq8jAOuCI/Ll2Hp2FhUggwUDXFsb8F7bope53TUJgWthHbTZ", "PQ5SSaxTSuJzj647muMyUtZ+uYgTQgJ31c5fk186jfk=", zzbaVar, zzab, 29));
        arrayList.add(new zzeb(zzdmVar, "Yt5XpSAtwgzkLrmXQYrSRr3fMYrlcB9+S6GNMxeM/YPVx9v5CKvi8nPmgPLSujhj", "dR638FFygwsrTt/QflyOMGvXjf1GSNZ7znIlrDruhqQ=", zzbaVar, zzab, 5));
        arrayList.add(new zzeh(zzdmVar, "dn8+8pHjJ2wj5Gp1r/T7fEkOwzyz7KKq5AugUUrNOwEHnTcvhTP4W3g7fxLqTpFB", "c6GziBvAtJVQl8145kYNIiXglYZwc9tWD7nqr7hLy/A=", zzbaVar, zzab, 12));
        arrayList.add(new zzej(zzdmVar, "aPsscg+Qxy0iqv59xAu19ukICD9tJAt4EQOJODpj5Ak49GynbXKvHyrrGvZ8bZTH", "gvEkKyx3YjkW3LmeffZWM+wAodTj/mcVSvq1iQ03Xm0=", zzbaVar, zzab, 3));
        arrayList.add(new zzec(zzdmVar, "0MHmFOerm+OLO7DlwBWYWOfS31i3abNb7NII/p/cozDrwy0Qt4Hz6KjLmcNA2Uu1", "dd/st6q19rXcYPAfnaAl9NStsvK8rw8184B9GIFsJ/U=", zzbaVar, zzab, 44));
        arrayList.add(new zzeg(zzdmVar, "Kl5uWPRlOPd31gHSoBTxQgFz93Wgsh3JQ5+syp0rqA1C1BUyIJ1EfI70uCpXJEXY", "MmO0rCvC3l83tbAoIwnFF0x6ASHVe9zZRiunMzgf61w=", zzbaVar, zzab, 22));
        arrayList.add(new zzeq(zzdmVar, "VN0mjb52HdSTqOivTj5aML5bEfMpu2JdQlvUopb/cybtBLaro7TgCOYdeQg7NyAj", "gp0cIgUUdAIZbRXtUI5gQGevY7Ql5nC5BzKn6LnObLY=", zzbaVar, zzab, 48));
        arrayList.add(new zzdy(zzdmVar, "cma5ZMQhKxelkLoCs6AZIf+jHy8LfWoLKTm3I25QQ7y8l7CIShnEfyEmDBtS6X1R", "8HMIy/Wfi2fsqmlkEx1MOaO07gLN9KPbPeJd9GTSqvM=", zzbaVar, zzab, 49));
        arrayList.add(new zzen(zzdmVar, "Jr72ymQHv5OslUBKT3bZCPBk5tCtKWt432zERI2guaDk8CCuhfLPuPGiolFM9O2V", "zYGiKyXCQARtrMz4+R+cwjpghR9+uCtdZ+gYsXH0O9M=", zzbaVar, zzab, 51));
        if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrk)).booleanValue()) {
            arrayList.add(new zzel(zzdmVar, "MT46CBRZPOofnotcmzgUNA75mBeAcqcqzo80kU1Wki/M3T0+Ilf2BPD/W1SRuWf8", "85aVi3SJCzURUruw80xwOCJlrH71CKBJVp0z1HeuVQI=", zzbaVar, zzab, 61));
        }
        return arrayList;
    }

    @Override // com.google.android.gms.internal.zzcs
    protected final zzdr zzb(MotionEvent motionEvent) throws zzdj {
        Method zza = zzagq.zza("Ncqp5H+22w9W3L8txFxikPL++CvvT4UACmJT2y9HNWRLTaCz/GhnD086ih+pDclQ", "oClLCKzkLLWAqjtGJeQeLZr6xaB9RrnEuNDXspc7PYI=");
        if (zza == null || motionEvent == null) {
            throw new zzdj();
        }
        try {
            return new zzdr((String) zza.invoke(null, motionEvent, this.zzaho));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new zzdj(e);
        }
    }

    @Override // com.google.android.gms.internal.zzcs, com.google.android.gms.internal.zzcr
    public final void zzb(View view) {
        if (((Boolean) zzlc.zzio().zzd(zzoi.zzbrm)).booleanValue()) {
            if (this.zzahu == null) {
                this.zzahu = new zzdu(zzagq, view);
            } else {
                this.zzahu.zzd(view);
            }
        }
    }
}
